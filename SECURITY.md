...
are not wrong to: the same behaviours — reach the network, run an installer, edit an
agent's settings file — are what a credential stealer does. The rating
...
### 1. It writes to Claude Code's settings
...
skill-sync makes no network call of its own accord: every one above starts from a command
you ran or a hook you installed.

### 3. It downloads and installs binaries
...
- No login items or background processes, and no self-update: it never downloads or
  replaces its own code. Versions before 2.5.0 could install an opt-in login watcher;
  the next session start deletes any entry they left.
- Remote data is not trusted: `manifest.json` is sanitised on read (unsafe names dropped,
  control and bidi characters stripped, lengths capped) before it reaches the terminal
  or an agent.
...
- **Zip-slip protection** — `provision` validates every archive member's path
  and extract them one at a time, resolved against the destination.
- **Download size cap** — 128 MB for tool downloads.
...
skill-sync then does nothing at all until you run a command.
